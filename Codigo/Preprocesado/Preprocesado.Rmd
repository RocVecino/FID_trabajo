...
```{r}
# Instalamos e iniciamos el paquete tidyverse
#install.packages("tidyverse")
library(tidyverse)

# Instalamos e iniciamos el paquete mice
...
```{r}
df<-read.csv("./StudentsPerformance.csv",stringsAsFactors = TRUE
...
Tras obtener todos los porcentajes relativos a los factores, los mostramos de manera conjunta.
...
Como se puede comprobar al ver los datos, la mayoría de las personas que han aprobado en cada uno de los tests ha tenido un almuerzo normal antes del examen.
...
Como se puede observar, la mayor parte de los estudiantes que han superado los tests se tratan de personas que no han recibido preparación previa, siguiendo la tendecia del número global de cada una de estas categorías.
...
Como se puede observar, la media de las mujeres en lectura y redacción es superior, pero en matemáticas los hombres tienen una media superior.
...
Como se puede observar, el grupo que ha obtenido la mejor media en todas los tests es el grupo e, seguido por d, c, b y a, en ese orden. Cabe destacar también que cada grupo ha obtenido la peor media en matemáticas excepto el grupo e.
...
Como se puede observar, las medias obtenidas en lectura y redacción son similares en dentro de cada grupo, teniendo normalmente una mayor diferencia con matemáticas.
...
Como se puede observar, los estudiantes con preparación previa han obtenido de media mejores resultados en todos los tests, aunque hay una mayor diferencia con matemáticas respecto al de tests.
...
write.csv(final_data,"./Prep_StudentsPerformance.csv", row.names = FALSE)
```
